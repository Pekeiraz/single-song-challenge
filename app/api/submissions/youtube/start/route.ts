...
import { parseYouTubePlaylistId, youtubeAuthorizeUrl } from "@/lib/youtube";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const playlistId = parseYouTubePlaylistId(String(playlistUrl ?? ""));
    const iat = Date.now();
    const state = createOAuthState({ provider: "youtube", challengeId, playlistId, nonce: crypto.randomBytes(24).toString("hex"), iat, exp: iat + 10 * 60 * 1000 });
    const response = NextResponse.json({ authorizeUrl: youtubeAuthorizeUrl(state) });
    response.cookies.set("youtube_oauth_state", state, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 600 });
    return response;
  } catch (error) {
