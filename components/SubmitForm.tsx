...
export function SubmitForm({ challengeId, providers }: { challengeId: string; providers: Record<Provider, boolean> }) {
  ...
  const detectedLabel = detected === "youtube" ? "YouTube" : detected === "spotify" ? "Spotify" : null;

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!detected) {
      setHasError(true);
      setMessage("Paste a Spotify or YouTube playlist link to continue.");
      return;
    }
    if (!providers[detected]) {
      setHasError(true);
      setMessage(`${detected === "youtube" ? "YouTube" : "Spotify"} imports are not enabled.`);
      return;
    }
    setHasError(false);
    setMessage("Preparing playlist …");
    const endpoint = detected === "youtube" ? "/api/submissions/youtube/start" : "/api/submissions/start";
    try {
  ...
  return <div className="card">
    ...
    <form onSubmit={submit} noValidate>
      <input value={url} onChange={(e) => { setUrl(e.target.value); if (hasError) setHasError(false); }} placeholder="Paste a Spotify or YouTube playlist link…" aria-invalid={hasError} style={hasError ? { borderColor: "#e5484d" } : undefined} />
      <br/><br/>
